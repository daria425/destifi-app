...
from app.config.ai_project_config import project_client
load_dotenv()
travel_agent_id=os.getenv("TRAVEL_AGENT_ID")
...
class TravelAgent:
    ...
    def respond_to_user(self, user_message:str, thread_id:str):
        self.thread_id=thread_id
        chat_instructions="You are a travel AI assistant that helps users plan vacations. Your goal is to provide detailed and personalized itineraries based on user requests. You should use your knowledge of travel trends, geographic conditions, and seasonal insights to create practical and well-structured recommendations. Ensure that the itineraries are aligned with the user's preferences and break down activities day by day. Make sure to ask clarifying questions if any details are missing and ensure that activities are specific."
        agent=self.project_client.agents.get_agent(assistant_id=self.agent_id)
        if agent.instructions!=chat_instructions or agent.tools:
            self.start_chat()
        message = self.project_client.agents.create_message(
            thread_id=self.thread_id,
            role="user",
    ...
    def delete_agent(self):
        self.project_client.agents.delete_agent(agent_id=self.agent_id)
        print(f"Deleted agent {self.agent_id} 🧚‍♀️")
